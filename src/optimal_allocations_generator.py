...
for instance in seeds_instances:
    ...
    if model_name == 'GSVM':
        ...
        auction_instance = PySats.getInstance().create_gsvm(seed=instance, isLegacyGSVM=isLegacy)  # (1)
    elif model_name == 'LSVM':
        print(model_name + str(instance))
        auction_instance = PySats.getInstance().create_lsvm(seed=instance, isLegacyLSVM=isLegacy) # (1)
    ...
    print('WDP SATS solved in ' + time.strftime('%H:%M:%S', time.gmtime(end-start)), '\n')
    print('Allocation:')
...
print()
print('CHECK:')
print('allocations equal:', a['Instance_Seed {}'.format(seed_instance)][0] == allocation_SATS)
